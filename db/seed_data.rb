USERS = [
  {
    username: "denholm",
    email: "denholm@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "denholm.jpg"
  },
  {
    username: "douglas",
    email: "douglas@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "douglas.jpg"
  },
  {
    username: "harold",
    email: "harold@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "harold.jpg"
  },
  {
    username: "jenn",
    email: "jenn@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "jenn.jpg"
  },
  {
    username: "moss",
    email: "moss@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "moss.jpg"
  },
  {
    username: "roy",
    email: "roy@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "roy.jpg"
  },
  {
    username: "ocha",
    email: "ocha@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "ocha.jpg"
  },
  {
    username: "Refeeld",
    email: "Refeeld@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "Refeeld.jpg"
  },
  {
    username: "C4C",
    email: "C4C@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "C4C.jpg"
  },
  {
    username: "ChilledCow",
    email: "ChilledCow@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "ChilledCow.jpg"
  },
  {
    username: "Chill Music",
    email: "Chill Music@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "Chill Music.jpg"
  },
  {
    username: "Sweet Medicine 甘い薬",
    email: "Sweet Medicine 甘い薬@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "Sweet Medicine 甘い薬.jpg"
  },
  {
    username: "Psalm//Trees",
    email: "Psalm//Trees@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "Psalm//Trees.jpg"
  },
  {
    username: "Chillhop Music",
    email: "Chillhop Music@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "Chillhop Music.jpg"
  },
  {
    username: "ghoulishmusic",
    email: "ghoulishmusic@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "ghoulishmusic.jpg"
  },
  {
    username: "cyberchase",
    email: "cyberchase@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "cyberchase.jpg"
  },
  {
    username: "dybredly",
    email: "dybredly@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "dybredly.jpg"
  },
  {
    username: "livo",
    email: "livo@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "livo.jpg"
  },
  {
    username: "Powfu",
    email: "Powfu@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "Powfu.jpg"
  },
  {
    username: "guest",
    email: "guest@chillabit.com",
    password: "password",
    avatar: "url",
    avatar_name: "guest.jpg"
  }
]

TRACK_FILENAME = [
  "Cozy Days (feat. mell-ø)",
  "Melted w/ Hazy Year (Chillhop Winter Essentials 2019)",
  "WYS - North Pole",
  "Ｈｏｍｅ － Ｒｅｓｏｎａｎｃｅ （Heal's Vaporwave Edit）",
  "sweet medicine - happy. - lofi hip hop beats",
  "God Would Be Missing Me (Vinyl available via Link)",
  "No Spirit - Snacks [full EP]",
  "Kudasai - The Girl I Haven't Met",
  "patience",
  "nobody belongs anywhere, nobody exists on purpose, everybody's gonna die",
  "ｆｅｅｌｉｎｇ ｋｉｎｄａ ｇｒａｙ",
  "death bed | Feat. beabadoobee"
]

TRACKS = [
  {
    name: "Cozy Days (feat. mell-ø)",
    artist: "Refeeld",
    artist_id: 7,
    genre: "Chillhop",
    tags: [],
    desc: "Special Thanks!!!
          @dreamhopp

          Thanks for the made this EP with me!
          @wyslofi
          @mellomusicnl
          @c4cmusic
          @harupandi

          Artwork by
          @ochakun

          Released by:
          Dreamhop Music
          Release date:
          17 November 2019",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_Refeeld-Cozy_Days.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[0] + '.jpg'
  },
  {
    name: "Melted w/ Hazy Year (Chillhop Winter Essentials 2019)",
    artist: "C4C",
    artist_id: 8,
    genre: "chillhop",
    tags: ["hazy year", "c4c", "melted", "chillhop", "guitar", "flute", "kalimba", "onlylove"],
    desc: "☃️ Vinyl / Stream » chillhop.ffm.to/winter2019.pbt

          A fine selection of warm beats to keep you warm until Spring. Thanks for joining in.

          shout out to my homie, the one and only, @hazy-year ! please give him a follow, he deserves it <3
          thanks again to @chillhopdotcom who made that possible :)

          onelove",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_C4C-Melted_w_Hazy_Year.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[1] + '.jpg'
  },
  {
    name: "WYS - North Pole",
    artist: "ChilledCow",
    artist_id: 9,
    genre: "chill beats",
    tags: ["lofi", "Hip Hop"],
    desc: "💿 Stream/Support
          → fanlink.to/wys-northpole

          North Pole by the talented french beatmaker WYS sets the mood up for truly nostalgic and reminiscing vibes. Guaranteed to take you on a journey. Just close your eyes and drift away ❄️

          Tracklist :
          [00:00] Ice Field
          [02:47] Comforting You
          [05:57] Satellite
          [09:46] Take Me Back
          [12:40] Shield

          🎵 Music by WYS
          → @wyslofi
          → open.spotify.com/artist/2CiO7xWdwPMDlVwlt9qa1f
          → wys-music.bandcamp.com/

          🎨 Artwork by Margaux Peltat
          → www.instagram.com/gomar_illustration/
          → www.patreon.com/margauxpeltat

          🎼 Listen to ChilledCow everywhere
          → bit.ly/chilledcow-spotify
          → bit.ly/chilledcow-applemusic
          → bit.ly/chilledcow-soundcloud",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_WYS-North_Pole.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[2] + '.jpg'
  },
  {
    name: "Ｈｏｍｅ － Ｒｅｓｏｎａｎｃｅ （Heal's Vaporwave Edit）",
    artist: "Chill Music",
    artist_id: 10,
    genre: "Vaporwave",
    tags: ["Vaporwave", "Chill", "Vibe"],
    desc: "Vibes.",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_Chill_Music-Home.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[3] + '.jpg'
  },
  {
    name: "sweet medicine - happy. - lofi hip hop beats",
    artist: "Sweet Medicine 甘い薬",
    artist_id: 11,
    genre: "Triphop",
    tags: ["Lofi Hip Hop Beats", "Lofi Hip Hop", "Lofi Beats"],
    desc: "Lofi Hip Hop",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_sweet_medicine-happy.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[4] + '.jpg'
  },
  {
    name: "God Would Be Missing Me (Vinyl available via Link)",
    artist: "Psalm//Trees",
    artist_id: 12,
    genre: "ydkwli",
    tags: [],
    desc: "Less than 100 Vinyl left.

          Cop here : psalmtreesbeats.bandcamp.com

          <3",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_Psalm_Trees-God_Would_Be_Missing_Me.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[5] + '.jpg'
  },
  {
    name: "No Spirit - Snacks [full EP]",
    artist: "Chillhop Music",
    artist_id: 13,
    genre: "Chillhop",
    tags: ["Hip Hop", "beats", "Instrumental", "Chill", "smooth", "relaxing"],
    desc: "🦜Download / Stream » chillhop.ffm.to/snacks.pbt

          00:00:00 No Spirit - Let's Stay Inside
          00:02:44 No Spirit x Philanthrope - Pacific
          00:05:06 No Spirit x dryhope - Aerials
          00:07:27 No Spirit x Kyle McEvoy, Philanthrope, Louk - Snacks Pt. 1
          00:10:24 No Spirit - Peaceful Sleep

          🎶 Listen to chillhop on Spotify & Apple Music
          ・ chillhop.lnk.to/sptfySi (Spotify)
          ・ chillhop.lnk.to/applemusic (Apple Music)

          🙏Follow No Spirit
          ・ @nospiritmusic
          ・ spoti.fi/3735gtO
          ・ instagram.com/nospiritmusic/

          🙌Follow Philanthrope
          ・@philanthrope1
          ・instagram.com/philanthrope__
          ・facebook.com/PhilanthropeMusic
          ・philanthrope.bandcamp.com
          ・spoti.fi/2G6We3r

          🙌Follow dryhope
          ・ @dryhope
          ・ instagram.com/dryhope_/
          ・ spoti.fi/36yHh60

          🙌Follow Kyle McEvoy
          ・ spoti.fi/2z9wz6F
          ・ instagram.com/kylemcevoymusic/
          ・ kylemcevoy.bandcamp.com
          ・ @kylemcevoymusic
          ・ twitter.com/kylemcevoymusic

          🙌Follow Louk
          ・ spoti.fi/2XWxyC6
          ・ @atx_dutch
          ・ instagram.com/atxdutch

          🎨Artwork by Jim Spendlove
          ・ instagram.com/jim_spendlove/
          ・ jimspendlove.myportfolio.com",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_Chillhop_Music-No_Spirit.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[6] + '.jpg'
  },
  {
    name: "Kudasai - The Girl I Haven't Met",
    artist: "ghoulishmusic",
    artist_id: 14,
    genre: "Falling EP",
    tags: ["lofi", "Hip Hop", "chill", "vibes", "study", "SCFIRST"],
    desc: 'twitter.com/kudasaibeats
          Thank you guys so much for 10k+ on "a night together"
          I love you all
          kudasaibeats.bandcamp.com/track/the-gi…i-havent-met',
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_Kudasai-The_Girl_I_Haven't_Met.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[7] + '.jpg'
  },
  {
    name: "patience",
    artist: "cyberchase",
    artist_id: 15,
    genre: "",
    tags: ["chill", "relax", "study", "homework", "laid back", "xxxtentacion", "Tag:", "lil", "uzi", "vert", "post", "malone", "peep", "skies", "cardi", "b", "rich", "the", "kid", "ski", "mask", "slump", "god", "lofi", "sad", "future", "edm", "Dance"],
    desc: "sampled an x song",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_cyberchase-patience.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[8] + '.jpg'
  },
  {
    name: "nobody belongs anywhere, nobody exists on purpose, everybody's gonna die",
    artist: "dybredly",
    artist_id: 16,
    genre: "M O R T I F I E D",
    tags: ["Lo-Fi", "lofi.hiphop", "Rick and Morty", "Guitar", "Hip Hop", "Bootleg boy"],
    desc: "Shoutout to @dabootlegboy for featuring this on his vol 4 <3
          www.youtube.com/watch?v=2OLeKlQ3QGE",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_dybredly-nobdy.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[9] + '.jpg'
  },
  {
    name: "ｆｅｅｌｉｎｇ ｋｉｎｄａ ｇｒａｙ",
    artist: "livo",
    artist_id: 17,
    genre: "lofi.hiphop",
    tags: ["lofi", "Hip Hop", "bumps", "Beats", "Lo-Fi", "lofihiphop", "kicks", "snare", "dilla", "Relax", "study", "Chill", "sleep", "sad", "anime", "jazz", "rhodes", "Loop"],
    desc: "livo.bandcamp.com/track/-",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_livo-feeling_gray.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[10] + '.jpg'
  },
  {
    name: "death bed | Feat. beabadoobee",
    artist: "Powfu",
    artist_id: 18,
    genre: "Lofihiphop",
    tags: ["lovesongs", "sad", "rap", "scfirst", "romantic"],
    desc: "Now on SPOTIFY & APPLE etc.

          Produced by: @otterpop123 Feat. @beabadoobee
          Suggested from Martin <3

          Lyrics:

          Hook

          Dont stay awake for too long
          Dont go to bed
          Ill make a cup of coffee for your head
          Ill get u up and going out of bed

          v1
          I don’t wanna fall asleep
          I don’t wanna pass away
          Thinking of r future cuz
          Ill never see those days

          I don’t know why this has happened
          But I probably deserve it
          I tried to do my best
          But u know that I’m not perfect

          Ive been praying for forgiveness
          you’ve been praying for my health
          when I leave this earth
          Hoping you’ll find somebody else

          Cuz ya we still young
          Theres so much we haven’t done
          Getting married
          Start a family
          Watch ur husband with his son

          I wish it could be me
          But I won’t make it off this bed
          Hope I go to heaven so I see u once again

          Ya my life was kind of short
          But I got so many blessings
          happy u were mine
          it sucks that its all ending

          v2
          Im happy that u here w me
          Sorry if I tear up
          When me and u were younger
          U would always make me cheer up

          taking goofy videos
          And walking through the park
          U would jump into my arms
          Every time u heard a bark

          We would cuddle in your sheets
          Sing me sound asleep
          And sneak out through your kitchen at exactly 1:03

          Sundays go to church
          Mondays watch a movie
          soon you’ll be alone
          Sorry that u have to loose me",
    cover: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_Powfu-death_bed.jpg",
    file: "https://chillabit-seeds.s3-us-west-1.amazonaws.com/track_file_ocha-iridscnt.mp3",
    filename: TRACK_FILENAME[11] + '.jpg'
  },
]